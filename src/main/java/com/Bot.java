...
public class Bot extends TelegramLongPollingBot {
    ...
    private void createKeyboard(SendMessage message, String[] buttons) {
        ...
        ArrayList<String> row1 = Arrays.stream(buttons).limit(buttons.length / 2).collect(Collectors.toCollection(ArrayList::new));
        ArrayList<String> row2 = Arrays.stream(buttons).skip(buttons.length / 2).collect(Collectors.toCollection(ArrayList::new));
